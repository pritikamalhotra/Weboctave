...
#      $this->plotcommand = file_get_contents("./proc/__wio_start.m");
  ...
  public function NewQuery($query) 
  {
    ...
    if(CF_USE_WRAPPER)
    {
      // when Octave will be run, user's dir will be .
      $query = "\n\n system(\"userdir .\");system(\"octadir ".CF_OCTAVE_USR_DIR."\");\nsystem(\"lock\");\n\n".$query;
    }
    ...
    $octruncmd="cd ".$this->user_dir.";ulimit -f ".$maxsize." -m 10000 -t ".$maxtime."; ";
    if(CF_USE_WRAPPER)
      $octruncmd=$octruncmd."LD_PRELOAD=".CF_WRAPPER_SO." ";
    $octruncmd=$octruncmd.CF_OCTAVE_EXECUTABLE." -H --no-line-editing "
               .OCTAVE_IN." > ".OCTAVE_OUT." 2> ".OCTAVE_ERR."; rm octave-core";
    passthru($octruncmd);
