...
class Security
{
  ...
  private $dangerous_strings  = array("fopen", "fclose", "system", "cd", "builtin", "link", "symlink", "readlink", "unlink", "readdir", "mkdir", "rmdir", "mkfifo", "unmask", "stat", "lstat", "fileattrib", "movefile", "copyfile", "tempname", "P_tmpdir");///< strings that we think to be dangerous
  ...
  private function BasicCuts($string)
  {
    ...
    if(!CF_USE_WRAPPER) //if wrapper is not in use, we fall back to old method
    {
  ...
  /// Doubles '\' in a \a string, for backslashes to be visible on the output
  /// \return \a string with backslashes doubled
  private function DoubleBackslashes($string)
  {
    $string = str_replace("\\", "\\\\", $string);
    return $string;
  }
  ...
  {
    // for A\x to be visible as A\x, not Ax
    $string = $this->DoubleBackslashes($string);
